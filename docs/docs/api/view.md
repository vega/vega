...
- *scheduling*: Opt-in cooperative scheduling of dataflow evaluation and canvas rendering (`boolean` or `{signal?: AbortSignal}`). When enabled, long-running [runAsync](#view_runAsync) calls periodically yield to the browser event loop (via `scheduler.yield()` where available, otherwise a `setTimeout` fallback) so that large datasets do not block the main thread, and canvas output is drawn to an offscreen buffer that is displayed once rendering completes. Evaluation yields between transforms, and also within the per-item loops of the data join, encoding and bounds transforms, so a single large mark does not hold the main thread for the whole of its own work. Line and area marks are still bounded as a unit, so a single such mark with very many points remains one uninterrupted task. Yields inherit the task priority of the caller; Vega never posts tasks with an explicit priority. If an `AbortSignal` is provided and aborted, the in-flight `runAsync` promise rejects with the signal's reason and the view should then be finalized. Calling [finalize](#view_finalize) also cancels any in-flight scheduled evaluation or rendering. While scheduling is enabled, synchronous [run](#view_run) calls are queued behind any in-flight evaluation and rendering rather than starting immediately. By default (`false`), evaluation and rendering run synchronously exactly as before.
