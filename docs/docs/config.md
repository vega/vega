...
| locale        | {% include type t="Object" %}        | Locale definitions for string parsing and formatting of number and date values. The locale object should contain `number` and/or `time` properties with [locale definitions](docs/api/locale). Locale definitions provided in the config block may be overridden by the View constructor *locale* option. {% include tag ver="5.12" %} |
