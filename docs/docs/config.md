...
| container     | {% include type t="Boolean" %}        | Configuration control for event listeners for the `"container"` source, which reports size changes of the DOM element containing the view. One of `true` (default) to permit container event listeners, or `false` to disallow them. {% include tag ver="6.4" %}|
