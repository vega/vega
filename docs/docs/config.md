...
| container     | {% include type t="Boolean" %}        | Configuration control for event listeners for the `"container"` source, which reports size changes of the DOM element containing the view. One of `true` (default) to permit container event listeners, or `false` to disallow them. {% include tag ver="6.3" %}|
...
| frame                 | {% include type t="String" %}   | The reference frame for the anchor position, one of `"group"` (the default, to anchor relative to the group width or height) or `"bounds"` (to anchor relative to the bounding box of the chart including axes; legends themselves and overflowing marks are not included). This property only applies to legends with an *orient* value of `"left"`, `"right"`, `"top"`, or `"bottom"`. |
