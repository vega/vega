...
| outline       | {% include type t="String" %} | The outline CSS shorthand property sets most of the focus outline properties in a single declaration. Any valid [CSS outline](https://developer.mozilla.org/en-US/docs/Web/CSS/outline) value can be used. This property is useful for suppressing the browser's default focus outline in favor of a focus outline rendered within the vega scenegraph (SVG output only). |
