var expr = require('../../src/parse/expr'),
    Graph = require('../../src/dataflow/Graph');

describe('Expression', function() {

  describe('Parse', function() {
    function parse(str) { return function() { expr(str); } }
    ...
    it('should not allow calling non-whitelisted expressions', function() {
      expect(parse("a()")).to.throw();
      expect(parse("a(0,1,2)")).to.throw();
      expect(parse("A()")).to.throw();
      expect(parse("A(0,1,2)")).to.throw();
      expect(parse("foo.bar(0,1,2)")).to.throw();
    });
  ...
  describe('Evaluate', function() {
    ...
    it('should not allow top-level identifiers outside whitelist', function() {
      expect(evaluate('Math')).to.be.undefined;
      expect(evaluate('Array')).to.be.undefined;
      expect(evaluate('String')).to.be.undefined;
    ...
    it('should not allow eval', function() {
      expect(evaluate('eval')).to.be.undefined;
    ...
    it('should not allow this reference', function() {
      expect(evaluate('this')).to.be.undefined;
    });

    it('should not allow arguments reference', function() {
      expect(evaluate('arguments')).to.be.undefined;
    });

    it('should not allow global variable reference', function() {
      expect(evaluate('window')).to.be.undefined;
      expect(evaluate('document')).to.be.undefined;
      expect(evaluate('self')).to.be.undefined;
      expect(evaluate('global')).to.be.undefined;
    });
  });
