describe('Lookup', function() {
  ...
  describe('Index Join', function() {
    var medalsVals = [
      {"country":"US", "gold":12, "silver":13, "bronze":15, "index":0},
      {"country":"Canada", "gold": 5, "silver": 4, "bronze": 3, "index":1}
    ...
    it('should handle streaming adds', function(done) {
      parseSpec(spec, function(model) {
        ...
        medalsDS.insert([{"country":"Mexico", "gold": 3, "silver": 3, "bronze": 2, "index": 2}]).fire();
        zip  = zipDS.values();
        medals = medalsDS.values();
        expect(zip).to.have.length(3);
        expect(medals).to.have.length(3);
  ...
  it('should validate against the schema', function() {
    var schema = schemaPath(transforms.lookup.schema),
        validate = validator(schema);

    expect(validate({ "type": "lookup", "on": "table",
    ...
    expect(validate({ "type": "lookup", "on": "table", "as": ["t"] })).to.be.false;
  });
