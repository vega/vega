...
const vega = require('../');
const inferType = vega.inferType;
const inferTypes = vega.inferTypes;
const typeParsers = vega.typeParsers;
...
tape('type parsers should parse date with format', function (t) {
  t.equal(+typeParsers.date('18.07.1990', d3.timeParse('%d.%m.%Y')), +new Date(1990, 6, 18));
  t.equal(+typeParsers.date('07.18.1990', d3.timeParse('%m.%d.%Y')), +new Date(1990, 6, 18));
