...
tape('TopoJSON reader should read TopoJSON mesh', function (t) {
  const mesh = read(worldText, {type: 'topojson', mesh: 'countries'});
...
tape('TopoJSON reader should read TopoJSON mesh interior', function (t) {
  const mesh = read(worldText, {type: 'topojson', mesh: 'countries', filter: 'interior'});
...
tape('TopoJSON reader should read TopoJSON mesh exterior', function (t) {
  const mesh = read(worldText, {type: 'topojson', mesh: 'countries', filter: 'exterior'});
  const tj = topojson.mesh(world, world.objects['countries'], (a, b) => a === b);
  t.equal(JSON.stringify(tj), JSON.stringify(mesh[0]));
  t.end();
});
...
tape('TopoJSON reader should read TopoJSON feature', function (t) {
  const feature = read(worldText, {type: 'topojson', feature: 'countries'});
