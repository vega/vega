...
export default function compare(
  ...
  const resolvedOpt: CompareOptions = opt || {};
  const resolvedOrders: readonly SortOrder[] = array(orders) || [];

  const ord: number[] = [], get: Accessor[] = [], fmap: Record<string, number> = {},
        gen = resolvedOpt.comparator || comparator;

  array(fields).forEach((f, i) => {
    if (f == null) return;
    const accessor = isFunction(f) ? f as Accessor : field(f as string, undefined, resolvedOpt);
    ord.push(resolvedOrders[i] === DESCENDING ? -1 : 1);
    get.push(accessor);
...
export const ascending = (_u: Comparable, _v: Comparable): -1 | 0 | 1 => {
  let u: Comparable = _u;
  let v: Comparable = _v;
  // Non-null assertions (!) are compile-time only and erased at runtime; JS safely compares null/undefined
