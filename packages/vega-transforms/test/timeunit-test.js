...
tape('TimeUnit truncates dates to time units', function (t) {
  ...
  const units = df.add(['year']);
  const c = df.add(Collect);
  const s = df.add(TimeUnit, {units: units, field: date, pulse: c});
...
tape('TimeUnit truncates UTC dates to time units', function (t) {
  ...
  const units = df.add(['year']);
  const c = df.add(Collect);
  const s = df.add(TimeUnit, {units: units, timezone: 'utc', field: date, pulse: c});
...
tape('TimeUnit supports unit steps', function (t) {
  ...
  const df = new vega.Dataflow();
  const date = field('date');
  const units = df.add(['year', 'month']);
  const step = df.add(1);
  const c = df.add(Collect);
  const s = df.add(TimeUnit, {units: units, step: step, field: date, pulse: c});
...
tape('TimeUnit supports unit inference', function (t) {
  ...
  const df = new vega.Dataflow();
  const date = field('date');
  const bins = df.add(12);
  const c = df.add(Collect);
  const s = df.add(TimeUnit, {field: date, pulse: c, maxbins: bins});
...
tape('TimeUnit supports unit inference with extent', function (t) {
  ...
  const date = field('date');
  const bins = df.add(12);
  const ext = df.add(util.extent(data, date));
  const c = df.add(Collect);
  const s = df.add(TimeUnit, {
    field: date,
    pulse: c,
    maxbins: bins,
    extent: ext
...
tape('TimeUnit supports point output', function (t) {
  ...
  const units = df.add(['year']);
  const c = df.add(Collect);
  const s = df.add(TimeUnit, {units: units, interval: false, field: date, pulse: c});
