var util = require('vega-util'), vega = require('vega-dataflow'), tx = require('../'), changeset = vega.changeset, Collect = tx.collect, Facet = tx.facet;

test('Facet facets tuples', function() {
  var data = [
  ...
  function subtest(len) {
    return function(s, i) {
      var d = s.data.value;
      expect(d.length).toBe(len===undefined ? i+1 : len);
      expect(d.every(function(t) { return t.k === s.key; })).toBe(true);
    };
  }
  ...
  expect(facet.targets().active).toBe(2); // 2 subflows updated
...
test("Facet handles key parameter change", function() {
  var data = [
  ...
  function subtest(len) {
    return function(s, i) {
      var d = s.data.value;
      expect(d.length).toBe(len===undefined ? i+1 : len);
      expect(d.every(function(t) { return t.k2 === s.key; })).toBe(true);
    };
  }
  ...
  df.touch(facet).run();
  expect(facet.targets().active).toBe(2); // 2 subflows updated
  expect(subs.length).toBe(3); // 3 subflows exist
  subs.forEach(subtest(2)); // subflows should have 2 tuples each
});

test('Facet key cache does not leak memory', function() {
