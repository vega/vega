...
function measure(base) {
  return function (out) {
    const m = extend({init: '', add: '', rem: '', idx: 0}, base);
    m.out = out || base.name;
    return m;
  };
}
...
export function compileMeasures(agg, field) {
  ...
  const all = resolve(agg, true); // assume streaming removes may occur
  let init = 'var cell = this.cell; this.valid = 0; this.missing = 0;';
  let ctr = 'this.cell = cell; this.init();';
  let add = 'if(v==null){++this.missing; return;} if(v!==v) return; ++this.valid;';
  let rem = 'if(v==null){--this.missing; return;} if(v!==v) return; --this.valid;';
  let set = 'var cell = this.cell;';
