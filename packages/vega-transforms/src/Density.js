...
const mixture = {
  ...
  params: [
    {name: 'distributions', type: 'param', array: true, params: distributions},
...
Density.Definition = {
  ...
  params: [
    ...
    {name: 'distribution', type: 'param', params: distributions.concat(mixture)},
    {name: 'as', type: 'string', array: true, default: ['value', 'density']}
  ]
};

const prototype = inherits(Density, Transform);
...
prototype.transform = function (_, pulse) {
  ...
  if (!this.value || pulse.changed() || _.modified()) {
    ...
    const values = sampleCurve(method, domain, minsteps, maxsteps).map(v => {
      const tuple = {};
      tuple[as[0]] = v[0];
      tuple[as[1]] = v[1];
      return ingest(tuple);
