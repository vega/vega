...
function extend(_) {
  for (let x, k, i = 1, len = arguments.length; i < len; ++i) {
    x = arguments[i]; // eslint-disable-line prefer-rest-params
    for (k in x) {
      _[k] = x[k];
    }
  }
...
const BASE = {
  ...
  refs: {
    ...
    paint: {
      oneOf: [{$ref: '#/refs/color'}, {$ref: '#/refs/linearGradient'}, {$ref: '#/refs/radialGradient'}]
    },
    color: {
      oneOf: [
        {
...
const TEXT_BASELINE = ['alphabetic', 'top', 'middle', 'bottom', 'line-top', 'line-bottom'];

const TEXT_FONT_WEIGHT = ['normal', 'bold', 'bolder', 'lighter', 100, 200, 300, 400, 500, 600, 700, 800, 900];
