...
export default class CanvasRenderer extends Renderer {
  ...
  renderAsync(scene, markTypes) {
    ...
    return r._renderScheduled(scene, markTypes)
      ...
      .then(() => r._rendering)
      .then(() => r)
      .finally(() => { if (r._call === call) r._call = null; });
  }
  ...
  async drawAsync(ctx, scene, bounds, markTypes) {
    ...
    if (scene.clip) clip(ctx, scene);
    if (mark.drawAsync) {
      await mark.drawAsync.call(this, ctx, scene, bounds, markTypes);
    } else if (mark.nested) {
      mark.draw.call(this, ctx, scene, bounds, markTypes);
    } else {
      await this._drawItemsChunked(mark, ctx, scene, bounds);
    }
    if (scene.clip) ctx.restore();
  }

  async _drawItemsChunked(mark, ctx, scene, bounds) {
    const scheduler = this._scheduler,
          items = visitItems(scene),
          n = items.length;
