...
function parseScaleDomain(domain, spec, scope) {
  ...
  return domain.signal
    ...
    : (isArray(domain) ? explicitDomain : domain.fields ? multipleDomain : singularDomain)(domain, spec, scope);
}
...
function multipleDomain(domain, spec, scope) {
  ...
  const fields = domain.fields.reduce(function (dom, d) {
    d = isString(d) ? {data: data, field: d} : isArray(d) || d.signal ? fieldRef(d, scope) : d;
    dom.push(d);
    return dom;
  }, []);
...
function ordinalMultipleDomain(domain, scope, fields) {
  ...
  const counts = fields.map(function (f) {
...
function numericMultipleDomain(domain, scope, fields) {
  ...
  const extents = fields.map(function (f) {
    const data = scope.getData(f.data);
    if (!data) dataLookupError(f.data);
...
function parseScaleRange(spec, scope, params) {
  ...
  if (range.signal) {
    ...
    if (config && hasOwnProperty(config, range)) {
      ...
      range = isDiscrete(spec.type) ? [0, {signal: 'height'}] : [{signal: 'height'}, 0];
    } else {
