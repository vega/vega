...
inherits(Bound, Transform, {
  transform(_, pulse) {
    const view = pulse.dataflow,
          ...
          bound = entry.bound,
          modified = _.modified(),
          scheduler = pulse.pulses ? null : view._scheduler;
    ...
    if (entry.nested) {
      ...
      if (scheduler && mark.items.length > SCHEDULING_BATCH) {
        return boundAllChunked(pulse, mark, scheduler, visit);
      }
      ...
      if (scheduler && pulse.add.length + pulse.mod.length
          + (visit.rebound ? mark.items.length : 0) > SCHEDULING_BATCH) {
        return boundIncrementalChunked(pulse, mark, scheduler, visit);
      }
...
async function boundAllChunked(pulse, mark, scheduler, visit) {
  await visitChunked(
    pulse.materialize(pulse.MOD).mod, visit.dirty, scheduler, SCHEDULING_BATCH
  );
  mark.bounds.clear();
...
async function boundIncrementalChunked(pulse, mark, scheduler, visit) {
  await visitChunked(
    pulse.materialize(pulse.ADD).add, visit.add, scheduler, SCHEDULING_BATCH
  );
  await visitChunked(
    pulse.materialize(pulse.MOD).mod, visit.mod, scheduler, SCHEDULING_BATCH
  );
