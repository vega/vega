...
tape('Allowlist event property access (non-datum objects only)', t => {
  ...
  // Block properties NOT on the allowlist via dot notation
  t.throws(() => evaluate('event.window'), 'blocks non-allowlisted property via dot notation');
  t.throws(() => evaluate('event.document'), 'blocks non-allowlisted property via dot notation');
  t.throws(() => evaluate('event.constructor'), 'blocks constructor access');
  t.throws(() => evaluate('event.view'), 'blocks native UIEvent.view (returns Window)');
  t.throws(() => evaluate('event.__proto__'), 'blocks __proto__ access');

  // Block properties NOT on the allowlist via bracket notation
  const event = {window: 'test', cookie: 'test', view: 'test'};
  t.equal(evaluate('event["window"]', {}, event), undefined, 'blocks non-allowlisted property via bracket notation');
  t.equal(evaluate('event["cookie"]', {}, event), undefined, 'blocks non-allowlisted property via bracket notation');
  t.equal(evaluate('event["view"]', {}, event), undefined, 'blocks native UIEvent.view via bracket notation');
  ...
  const datum = {window: 'value1', document: 'value2', cookie: 'value3', constructor: 'value4'};
  ...
  t.equal(evaluate('datum.constructor', datum), 'value4', 'allows datum constructor access');
  t.equal(evaluate('datum["cookie"]', datum), 'value3', 'allows computed datum property access');

  // Non-sensitive literal objects should also be restricted (not datum)
  t.throws(() => evaluate('({window: 7}).window'), 'blocks non-allowlisted property on object literals');
