...
async function joinChunked(pulse, scheduler, visit) {
  await visitChunked(pulse.materialize(pulse.ADD).add, visit.add, scheduler, SCHEDULING_BATCH);
  await visitChunked(pulse.materialize(pulse.MOD).mod, visit.mod, scheduler, SCHEDULING_BATCH);
  await visitChunked(pulse.materialize(pulse.REM).rem, visit.rem, scheduler, SCHEDULING_BATCH);
  return visit.finish();
}
