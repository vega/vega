...
const Forces = 'forces';
const ForceParams = ['alpha', 'alphaMin', 'alphaTarget', 'velocityDecay', 'forces'];
...
const prototype = inherits(Force, Transform);

prototype.transform = function (_, pulse) {
  let sim = this.value;
  let change = pulse.changed(pulse.ADD_REM);
  const params = _.modified(ForceParams);
  let iters = _.iterations || 300;
...
prototype.finish = function (_, pulse) {
  ...
  for (let args = this._argops, j = 0, m = args.length, arg; j < m; ++j) {
    ...
    for (let ops = arg.op._argops, i = 0, n = ops.length, op; i < n; ++i) {
