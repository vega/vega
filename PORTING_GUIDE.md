# Vega 3 Porting Guide

This document describes the changes one needs to make to port Vega 2.x visualizations to the new 3.0 specification. It also introduces a subset of the new features provided by Vega 3.0.

- [Visual Encodings](#visual-encodings)
- [Mark Types and Scenegraph Rendering](#mark-types-and-scenegraph-rendering)
...
- The `"mult"` and `"offset"` visual encoding properties now support recursive encoding directives for more expressive layouts. For example, each can include a sub-encoding like so: `"x": {"scale": "xscale", "field": "foo", "offset": {"scale": "oscale", "field": "bar"}}`.

- The `"band"` property for encoding using a `"band"` (previously `"ordinal"`) scale now accepts an interpolation fraction between 0 and 1 for placing elements within a scale band, and may be used in conjunction with a field-based scale mapping. For example, the following encoding places an item at the midpoint of the appropriate scale band: `{"scale": "xscale", "field": "foo", "band": 0.5}`.

## Mark Types and Scenegraph Rendering
...
- Vega 3 has removes the `"auto"` option for view `"padding"`. Instead, `"padding"` now always defines a fixed padding margin around the visualization. If the `"autosize"` property is set to `"pad"`, the padding values will be added to the results of auto-size determination.
...
- The `"category10"`, `"category20"` and similar color palettes are no longer available as built-in range names. Instead, they are available using the scale `"scheme"` property, which can be specified instead of a scale range for `"ordinal"` and `"sequential"` scales. However, Vega 3 does support a built-in `"category"` short-hand for ordinal scale ranges, which can be re-defined depending on the current configuration.
...
- Vega 2.x transform `"output"` maps for determining which data tuple properties have been replaced by an `"as"` parameter that takes either a single string or an ordered array of strings, depending on the transform type.

- Mid-point calculations for layout transforms suchs as `"pie"`, `"stack"`, and `"bin"` are no longer included as output to improve efficiency. Instead, one can use an `"expr"` encoding to calulate a midpoint, or provide a nested `"offset"` encoding.
...
- Vega 3 also introduces a number of new transforms, and modifications to previous transforms (including a dramatically improved `"force"` transform and improved tree generation and layout transforms). Web-based documentation is still forthcoming, however, most of these transforms are demonstrated in the example specifications included in this repo. In addition, the parameters accepted by each transform are documented via JSDoc comments in the source code. Please consult the appropriate Vega module repositories for further information.

## Geo Data

- Cartographic projections are now defined in their own top-level block, similar to scales. Named projections are defined within an array under the `projections` property. Inline projection definitions in geo-transforms are no longer allowed. Instead, geo-transforms require a `"projection"` property whose value is the name of a defined projection.

- The `Geo` transform has been renamed `GeoPoint`.

- A Graticule transform has been added to generate reference lines for projections. To add graticules for a map, create a new data set definition with no URL or source properties, and include a `"graticule"` transform as the first entry in the data transforms array.
...
- Animated transitions are not currently supported by Vega 3.0. As a result, the `"delay"` and `"ease"` properties are not processed.

## View API

- The Vega View API has been updated, and in many ways simplified. To create a Vega visualization, the Vega spec must first be parsed, then the resulting *runtime spec* can be passed as an argument to the `View` constructor. Unlike Vega 2.x, this process does not require asynchronous callbacks. For example:
  ```js
...
- The underlying View API and dataflow system have been extensively overhauled. A Vega `View` instance is now a direct subclass of a Vega `Dataflow`, and manages all reactive processing, data loading and rendering. While full web-based documentation is still forthcoming, comprehensive JSDoc comments are included in the source code of both the [vega-view](https://github.com/vega/vega-view) and [vega-dataflow](https://github.com/vega/vega-dataflow) repositories.
