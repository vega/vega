...
export default function(commandLineArgs) {
  ...
  function commonPlugins(mode) {
    const targets = {
      ...(mode == 'node' ? {node: 'current'} : {}),
      ...(mode == 'esmodules' ? {esmodules: true} : {}),
      ...(mode == 'ie' ? {ie: '11'} : {})
    };
  ...
  const outputs = [{
    ...
    plugins: [nodePlugin(false), ...commonPlugins('node')]
  }, {
    ...
    plugins: [nodePlugin(true), ...commonPlugins('esmodules')]
  }];
  ...
  if (!commandLineArgs['config-node']) {
    outputs.push({
      ...
      plugins: [nodePlugin(true), ...commonPlugins()]
    });
    ...
    if (ie) {
      outputs.push({
        ...
        plugins: [nodePlugin(true), ...commonPlugins('ie')]
      });
    }
  }
